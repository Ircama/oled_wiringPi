#include <stdio.h>

// From https://github.com/CariadDisplayLibrary/VGA8x8
const unsigned char font8x8[] = {
    /* 0x00 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x01 */
    0x7E, 0x81, 0xA5, 0x81, 0xBD, 0x99, 0x81, 0x7E, 

    /* 0x02 */
    0x7E, 0xFF, 0xDB, 0xFF, 0xC3, 0xE7, 0xFF, 0x7E, 

    /* 0x03 */
    0x36, 0x7F, 0x7F, 0x7F, 0x3E, 0x1C, 0x08, 0x00, 

    /* 0x04 */
    0x08, 0x1C, 0x3E, 0x7F, 0x3E, 0x1C, 0x08, 0x00, 

    /* 0x05 */
    0x1C, 0x3E, 0x1C, 0x7F, 0x7F, 0x6B, 0x08, 0x1C, 

    /* 0x06 */
    0x08, 0x1C, 0x3E, 0x7F, 0x7F, 0x3E, 0x08, 0x1C, 

    /* 0x07 */
    0x00, 0x00, 0x18, 0x3C, 0x3C, 0x18, 0x00, 0x00, 

    /* 0x08 */
    0xFF, 0xFF, 0xE7, 0xC3, 0xC3, 0xE7, 0xFF, 0xFF, 

    /* 0x09 */
    0x00, 0x3C, 0x66, 0x42, 0x42, 0x66, 0x3C, 0x00, 

    /* 0x0A */
    0xFF, 0xC3, 0x99, 0xBD, 0xBD, 0x99, 0xC3, 0xFF, 

    /* 0x0B */
    0xF0, 0xE0, 0xF0, 0xBE, 0x33, 0x33, 0x33, 0x1E, 

    /* 0x0C */
    0x3C, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x7E, 0x18, 

    /* 0x0D */
    0xFC, 0xCC, 0xFC, 0x0C, 0x0C, 0x0E, 0x0F, 0x07, 

    /* 0x0E */
    0xFE, 0xC6, 0xFE, 0xC6, 0xC6, 0xE6, 0x67, 0x03, 

    /* 0x0F */
    0x18, 0xDB, 0x3C, 0xE7, 0xE7, 0x3C, 0xDB, 0x18, 

    /* 0x10 */
    0x01, 0x07, 0x1F, 0x7F, 0x1F, 0x07, 0x01, 0x00, 

    /* 0x11 */
    0x40, 0x70, 0x7C, 0x7F, 0x7C, 0x70, 0x40, 0x00, 

    /* 0x12 */
    0x18, 0x3C, 0x7E, 0x18, 0x18, 0x7E, 0x3C, 0x18, 

    /* 0x13 */
    0x66, 0x66, 0x66, 0x66, 0x66, 0x00, 0x66, 0x00, 

    /* 0x14 */
    0xFE, 0xDB, 0xDB, 0xDE, 0xD8, 0xD8, 0xD8, 0x00, 

    /* 0x15 */
    0x7C, 0x86, 0x3C, 0x66, 0x66, 0x3C, 0x61, 0x3E, 

    /* 0x16 */
    0x00, 0x00, 0x00, 0x00, 0x7E, 0x7E, 0x7E, 0x00, 

    /* 0x17 */
    0x18, 0x3C, 0x7E, 0x18, 0x7E, 0x3C, 0x18, 0xFF, 

    /* 0x18 */
    0x18, 0x3C, 0x7E, 0x18, 0x18, 0x18, 0x18, 0x00, 

    /* 0x19 */
    0x18, 0x18, 0x18, 0x18, 0x7E, 0x3C, 0x18, 0x00, 

    /* 0x1A */
    0x00, 0x18, 0x30, 0x7F, 0x30, 0x18, 0x00, 0x00, 

    /* 0x1B */
    0x00, 0x0C, 0x06, 0x7F, 0x06, 0x0C, 0x00, 0x00, 

    /* 0x1C */
    0x00, 0x00, 0x03, 0x03, 0x03, 0x7F, 0x00, 0x00, 

    /* 0x1D */
    0x00, 0x24, 0x66, 0xFF, 0x66, 0x24, 0x00, 0x00, 

    /* 0x1E */
    0x00, 0x18, 0x3C, 0x7E, 0xFF, 0xFF, 0x00, 0x00, 

    /* 0x1F */
    0x00, 0xFF, 0xFF, 0x7E, 0x3C, 0x18, 0x00, 0x00, 

    /* 0x20 */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x21 */
    0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, 0x18, 0x00, 

    /* 0x22 */
    0x66, 0x66, 0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x23 */
    0x36, 0x36, 0x7F, 0x36, 0x7F, 0x36, 0x36, 0x00, 

    /* 0x24 */
    0x18, 0x7C, 0x06, 0x3C, 0x60, 0x3E, 0x18, 0x00, 

    /* 0x25 */
    0x00, 0x63, 0x33, 0x18, 0x0C, 0x66, 0x63, 0x00, 

    /* 0x26 */
    0x1C, 0x36, 0x1C, 0x6E, 0x3B, 0x33, 0x6E, 0x00, 

    /* 0x27 */
    0x18, 0x18, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x28 */
    0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00, 

    /* 0x29 */
    0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00, 

    /* 0x2A */
    0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00, 

    /* 0x2B */
    0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00, 

    /* 0x2C */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x0C, 

    /* 0x2D */
    0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00, 

    /* 0x2E */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00, 

    /* 0x2F */
    0x60, 0x30, 0x18, 0x0C, 0x06, 0x03, 0x01, 0x00, 

    /* 0x30 */
    0x1C, 0x36, 0x63, 0x6B, 0x63, 0x36, 0x1C, 0x00, 

    /* 0x31 */
    0x18, 0x1C, 0x18, 0x18, 0x18, 0x18, 0x7E, 0x00, 

    /* 0x32 */
    0x3E, 0x63, 0x60, 0x38, 0x0C, 0x66, 0x7F, 0x00, 

    /* 0x33 */
    0x3E, 0x63, 0x60, 0x3C, 0x60, 0x63, 0x3E, 0x00, 

    /* 0x34 */
    0x38, 0x3C, 0x36, 0x33, 0x7F, 0x30, 0x78, 0x00, 

    /* 0x35 */
    0x7F, 0x03, 0x03, 0x3F, 0x60, 0x63, 0x3E, 0x00, 

    /* 0x36 */
    0x1C, 0x06, 0x03, 0x3F, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x37 */
    0x7F, 0x63, 0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x00, 

    /* 0x38 */
    0x3E, 0x63, 0x63, 0x3E, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x39 */
    0x3E, 0x63, 0x63, 0x7E, 0x60, 0x30, 0x1E, 0x00, 

    /* 0x3A */
    0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x00, 

    /* 0x3B */
    0x00, 0x18, 0x18, 0x00, 0x00, 0x18, 0x18, 0x0C, 

    /* 0x3C */
    0x60, 0x30, 0x18, 0x0C, 0x18, 0x30, 0x60, 0x00, 

    /* 0x3D */
    0x00, 0x00, 0x7E, 0x00, 0x00, 0x7E, 0x00, 0x00, 

    /* 0x3E */
    0x06, 0x0C, 0x18, 0x30, 0x18, 0x0C, 0x06, 0x00, 

    /* 0x3F */
    0x3E, 0x63, 0x30, 0x18, 0x18, 0x00, 0x18, 0x00, 

    /* 0x40 */
    0x3E, 0x63, 0x7B, 0x7B, 0x7B, 0x03, 0x1E, 0x00, 

    /* 0x41 */
    0x1C, 0x36, 0x63, 0x7F, 0x63, 0x63, 0x63, 0x00, 

    /* 0x42 */
    0x3F, 0x66, 0x66, 0x3E, 0x66, 0x66, 0x3F, 0x00, 

    /* 0x43 */
    0x3C, 0x66, 0x03, 0x03, 0x03, 0x66, 0x3C, 0x00, 

    /* 0x44 */
    0x1F, 0x36, 0x66, 0x66, 0x66, 0x36, 0x1F, 0x00, 

    /* 0x45 */
    0x7F, 0x46, 0x16, 0x1E, 0x16, 0x46, 0x7F, 0x00, 

    /* 0x46 */
    0x7F, 0x46, 0x16, 0x1E, 0x16, 0x06, 0x0F, 0x00, 

    /* 0x47 */
    0x3C, 0x66, 0x03, 0x03, 0x73, 0x66, 0x5C, 0x00, 

    /* 0x48 */
    0x63, 0x63, 0x63, 0x7F, 0x63, 0x63, 0x63, 0x00, 

    /* 0x49 */
    0x3C, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x4A */
    0x78, 0x30, 0x30, 0x30, 0x33, 0x33, 0x1E, 0x00, 

    /* 0x4B */
    0x67, 0x66, 0x36, 0x1E, 0x36, 0x66, 0x67, 0x00, 

    /* 0x4C */
    0x0F, 0x06, 0x06, 0x06, 0x46, 0x66, 0x7F, 0x00, 

    /* 0x4D */
    0x63, 0x77, 0x7F, 0x7F, 0x6B, 0x63, 0x63, 0x00, 

    /* 0x4E */
    0x63, 0x67, 0x6F, 0x7B, 0x73, 0x63, 0x63, 0x00, 

    /* 0x4F */
    0x3E, 0x63, 0x63, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x50 */
    0x3F, 0x66, 0x66, 0x3E, 0x06, 0x06, 0x0F, 0x00, 

    /* 0x51 */
    0x3E, 0x63, 0x63, 0x63, 0x63, 0x73, 0x3E, 0x70, 

    /* 0x52 */
    0x3F, 0x66, 0x66, 0x3E, 0x36, 0x66, 0x67, 0x00, 

    /* 0x53 */
    0x3C, 0x66, 0x0C, 0x18, 0x30, 0x66, 0x3C, 0x00, 

    /* 0x54 */
    0x7E, 0x7E, 0x5A, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x55 */
    0x63, 0x63, 0x63, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x56 */
    0x63, 0x63, 0x63, 0x63, 0x63, 0x36, 0x1C, 0x00, 

    /* 0x57 */
    0x63, 0x63, 0x63, 0x6B, 0x6B, 0x7F, 0x36, 0x00, 

    /* 0x58 */
    0x63, 0x63, 0x36, 0x1C, 0x36, 0x63, 0x63, 0x00, 

    /* 0x59 */
    0x66, 0x66, 0x66, 0x3C, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x5A */
    0x7F, 0x63, 0x31, 0x18, 0x4C, 0x66, 0x7F, 0x00, 

    /* 0x5B */
    0x3C, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x3C, 0x00, 

    /* 0x5C */
    0x03, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x40, 0x00, 

    /* 0x5D */
    0x3C, 0x30, 0x30, 0x30, 0x30, 0x30, 0x3C, 0x00, 

    /* 0x5E */
    0x08, 0x1C, 0x36, 0x63, 0x00, 0x00, 0x00, 0x00, 

    /* 0x5F */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 

    /* 0x60 */
    0x0C, 0x18, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x61 */
    0x00, 0x00, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0x62 */
    0x07, 0x06, 0x3E, 0x66, 0x66, 0x66, 0x3B, 0x00, 

    /* 0x63 */
    0x00, 0x00, 0x3E, 0x63, 0x03, 0x63, 0x3E, 0x00, 

    /* 0x64 */
    0x38, 0x30, 0x3E, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0x65 */
    0x00, 0x00, 0x3E, 0x63, 0x7F, 0x03, 0x3E, 0x00, 

    /* 0x66 */
    0x3C, 0x66, 0x06, 0x1F, 0x06, 0x06, 0x0F, 0x00, 

    /* 0x67 */
    0x00, 0x00, 0x6E, 0x33, 0x33, 0x3E, 0x30, 0x1F, 

    /* 0x68 */
    0x07, 0x06, 0x36, 0x6E, 0x66, 0x66, 0x67, 0x00, 

    /* 0x69 */
    0x18, 0x00, 0x1C, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x6A */
    0x60, 0x00, 0x60, 0x60, 0x60, 0x66, 0x66, 0x3C, 

    /* 0x6B */
    0x07, 0x06, 0x66, 0x36, 0x1E, 0x36, 0x67, 0x00, 

    /* 0x6C */
    0x1C, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x6D */
    0x00, 0x00, 0x37, 0x7F, 0x6B, 0x6B, 0x6B, 0x00, 

    /* 0x6E */
    0x00, 0x00, 0x3B, 0x66, 0x66, 0x66, 0x66, 0x00, 

    /* 0x6F */
    0x00, 0x00, 0x3E, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x70 */
    0x00, 0x00, 0x3B, 0x66, 0x66, 0x3E, 0x06, 0x0F, 

    /* 0x71 */
    0x00, 0x00, 0x6E, 0x33, 0x33, 0x3E, 0x30, 0x78, 

    /* 0x72 */
    0x00, 0x00, 0x3B, 0x6E, 0x06, 0x06, 0x0F, 0x00, 

    /* 0x73 */
    0x00, 0x00, 0x7E, 0x03, 0x3E, 0x60, 0x3F, 0x00, 

    /* 0x74 */
    0x0C, 0x0C, 0x3F, 0x0C, 0x0C, 0x6C, 0x38, 0x00, 

    /* 0x75 */
    0x00, 0x00, 0x33, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0x76 */
    0x00, 0x00, 0x63, 0x63, 0x63, 0x36, 0x1C, 0x00, 

    /* 0x77 */
    0x00, 0x00, 0x63, 0x6B, 0x6B, 0x7F, 0x36, 0x00, 

    /* 0x78 */
    0x00, 0x00, 0x63, 0x36, 0x1C, 0x36, 0x63, 0x00, 

    /* 0x79 */
    0x00, 0x00, 0x63, 0x63, 0x63, 0x7E, 0x60, 0x3F, 

    /* 0x7A */
    0x00, 0x00, 0x7E, 0x32, 0x18, 0x4C, 0x7E, 0x00, 

    /* 0x7B */
    0x70, 0x18, 0x18, 0x0E, 0x18, 0x18, 0x70, 0x00, 

    /* 0x7C */
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00, 

    /* 0x7D */
    0x0E, 0x18, 0x18, 0x70, 0x18, 0x18, 0x0E, 0x00, 

    /* 0x7E */
    0x6E, 0x3B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 

    /* 0x7F */
    0x00, 0x08, 0x1C, 0x36, 0x63, 0x63, 0x7F, 0x00, 

    /* 0x80 */
    0x3E, 0x63, 0x03, 0x03, 0x63, 0x3E, 0x30, 0x1E, 

    /* 0x81 */
    0x33, 0x00, 0x33, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0x82 */
    0x30, 0x18, 0x3E, 0x63, 0x7F, 0x03, 0x3E, 0x00, 

    /* 0x83 */
    0x3E, 0x41, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0x84 */
    0x63, 0x00, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0x85 */
    0x0C, 0x18, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0x86 */
    0x0C, 0x0C, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0x87 */
    0x00, 0x00, 0x7E, 0x03, 0x03, 0x7E, 0x30, 0x1C, 

    /* 0x88 */
    0x3E, 0x41, 0x3E, 0x63, 0x7F, 0x03, 0x3E, 0x00, 

    /* 0x89 */
    0x63, 0x00, 0x3E, 0x63, 0x7F, 0x03, 0x3E, 0x00, 

    /* 0x8A */
    0x0C, 0x18, 0x3E, 0x63, 0x7F, 0x03, 0x3E, 0x00, 

    /* 0x8B */
    0x66, 0x00, 0x1C, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x8C */
    0x3E, 0x41, 0x1C, 0x18, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x8D */
    0x0C, 0x18, 0x00, 0x1C, 0x18, 0x18, 0x3C, 0x00, 

    /* 0x8E */
    0x63, 0x1C, 0x36, 0x63, 0x7F, 0x63, 0x63, 0x00, 

    /* 0x8F */
    0x1C, 0x36, 0x3E, 0x63, 0x7F, 0x63, 0x63, 0x00, 

    /* 0x90 */
    0x18, 0x0C, 0x7F, 0x03, 0x1F, 0x03, 0x7F, 0x00, 

    /* 0x91 */
    0x00, 0x00, 0x7E, 0x18, 0x7E, 0x1B, 0x7E, 0x00, 

    /* 0x92 */
    0x7C, 0x36, 0x33, 0x7F, 0x33, 0x33, 0x73, 0x00, 

    /* 0x93 */
    0x3E, 0x41, 0x3E, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x94 */
    0x63, 0x00, 0x3E, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x95 */
    0x0C, 0x18, 0x3E, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x96 */
    0x1E, 0x21, 0x00, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0x97 */
    0x06, 0x0C, 0x33, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0x98 */
    0x63, 0x00, 0x63, 0x63, 0x63, 0x7E, 0x60, 0x3F, 

    /* 0x99 */
    0x63, 0x1C, 0x36, 0x63, 0x63, 0x36, 0x1C, 0x00, 

    /* 0x9A */
    0x63, 0x00, 0x63, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0x9B */
    0x18, 0x18, 0x7E, 0x03, 0x03, 0x7E, 0x18, 0x18, 

    /* 0x9C */
    0x1C, 0x36, 0x26, 0x0F, 0x06, 0x66, 0x3F, 0x00, 

    /* 0x9D */
    0x66, 0x66, 0x3C, 0x7E, 0x18, 0x7E, 0x18, 0x18, 

    /* 0x9E */
    0x1F, 0x33, 0x33, 0x5F, 0x63, 0xF3, 0x63, 0xE3, 

    /* 0x9F */
    0x70, 0xD8, 0x18, 0x3C, 0x18, 0x1B, 0x0E, 0x00, 

    /* 0xA0 */
    0x18, 0x0C, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00, 

    /* 0xA1 */
    0x30, 0x18, 0x00, 0x1C, 0x18, 0x18, 0x3C, 0x00, 

    /* 0xA2 */
    0x30, 0x18, 0x3E, 0x63, 0x63, 0x63, 0x3E, 0x00, 

    /* 0xA3 */
    0x18, 0x0C, 0x33, 0x33, 0x33, 0x33, 0x6E, 0x00, 

    /* 0xA4 */
    0x6E, 0x3B, 0x00, 0x3B, 0x66, 0x66, 0x66, 0x00, 

    /* 0xA5 */
    0x6E, 0x3B, 0x00, 0x67, 0x6F, 0x7B, 0x73, 0x00, 

    /* 0xA6 */
    0x3C, 0x36, 0x36, 0x7C, 0x00, 0x7E, 0x00, 0x00, 

    /* 0xA7 */
    0x1C, 0x36, 0x36, 0x1C, 0x00, 0x3E, 0x00, 0x00, 

    /* 0xA8 */
    0x18, 0x00, 0x18, 0x18, 0x0C, 0xC6, 0x7C, 0x00, 

    /* 0xA9 */
    0x00, 0x00, 0x00, 0x7F, 0x03, 0x03, 0x00, 0x00, 

    /* 0xAA */
    0x00, 0x00, 0x00, 0x7F, 0x60, 0x60, 0x00, 0x00, 

    /* 0xAB */
    0xC6, 0x67, 0x36, 0x7E, 0xCC, 0x66, 0x33, 0xF0, 

    /* 0xAC */
    0xC6, 0x67, 0x36, 0x5E, 0x6C, 0x56, 0xFB, 0x60, 

    /* 0xAD */
    0x18, 0x00, 0x18, 0x18, 0x3C, 0x3C, 0x18, 0x00, 

    /* 0xAE */
    0x00, 0xCC, 0x66, 0x33, 0x66, 0xCC, 0x00, 0x00, 

    /* 0xAF */
    0x00, 0x33, 0x66, 0xCC, 0x66, 0x33, 0x00, 0x00, 

    /* 0xB0 */
    0x44, 0x11, 0x44, 0x11, 0x44, 0x11, 0x44, 0x11, 

    /* 0xB1 */
    0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55, 0xAA, 0x55, 

    /* 0xB2 */
    0xEE, 0xBB, 0xEE, 0xBB, 0xEE, 0xBB, 0xEE, 0xBB, 

    /* 0xB3 */
    0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 

    /* 0xB4 */
    0x18, 0x18, 0x18, 0x18, 0x1F, 0x18, 0x18, 0x18, 

    /* 0xB5 */
    0x18, 0x18, 0x1F, 0x18, 0x1F, 0x18, 0x18, 0x18, 

    /* 0xB6 */
    0x6C, 0x6C, 0x6C, 0x6C, 0x6F, 0x6C, 0x6C, 0x6C, 

    /* 0xB7 */
    0x00, 0x00, 0x00, 0x00, 0x7F, 0x6C, 0x6C, 0x6C, 

    /* 0xB8 */
    0x00, 0x00, 0x1F, 0x18, 0x1F, 0x18, 0x18, 0x18, 

    /* 0xB9 */
    0x6C, 0x6C, 0x6F, 0x60, 0x6F, 0x6C, 0x6C, 0x6C, 

    /* 0xBA */
    0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 0x6C, 

    /* 0xBB */
    0x00, 0x00, 0x7F, 0x60, 0x6F, 0x6C, 0x6C, 0x6C, 

    /* 0xBC */
    0x6C, 0x6C, 0x6F, 0x60, 0x7F, 0x00, 0x00, 0x00, 

    /* 0xBD */
    0x6C, 0x6C, 0x6C, 0x6C, 0x7F, 0x00, 0x00, 0x00, 

    /* 0xBE */
    0x18, 0x18, 0x1F, 0x18, 0x1F, 0x00, 0x00, 0x00, 

    /* 0xBF */
    0x00, 0x00, 0x00, 0x00, 0x1F, 0x18, 0x18, 0x18, 

    /* 0xC0 */
    0x18, 0x18, 0x18, 0x18, 0xF8, 0x00, 0x00, 0x00, 

    /* 0xC1 */
    0x18, 0x18, 0x18, 0x18, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xC2 */
    0x00, 0x00, 0x00, 0x00, 0xFF, 0x18, 0x18, 0x18, 

    /* 0xC3 */
    0x18, 0x18, 0x18, 0x18, 0xF8, 0x18, 0x18, 0x18, 

    /* 0xC4 */
    0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xC5 */
    0x18, 0x18, 0x18, 0x18, 0xFF, 0x18, 0x18, 0x18, 

    /* 0xC6 */
    0x18, 0x18, 0xF8, 0x18, 0xF8, 0x18, 0x18, 0x18, 

    /* 0xC7 */
    0x6C, 0x6C, 0x6C, 0x6C, 0xEC, 0x6C, 0x6C, 0x6C, 

    /* 0xC8 */
    0x6C, 0x6C, 0xEC, 0x0C, 0xFC, 0x00, 0x00, 0x00, 

    /* 0xC9 */
    0x00, 0x00, 0xFC, 0x0C, 0xEC, 0x6C, 0x6C, 0x6C, 

    /* 0xCA */
    0x6C, 0x6C, 0xEF, 0x00, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xCB */
    0x00, 0x00, 0xFF, 0x00, 0xEF, 0x6C, 0x6C, 0x6C, 

    /* 0xCC */
    0x6C, 0x6C, 0xEC, 0x0C, 0xEC, 0x6C, 0x6C, 0x6C, 

    /* 0xCD */
    0x00, 0x00, 0xFF, 0x00, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xCE */
    0x6C, 0x6C, 0xEF, 0x00, 0xEF, 0x6C, 0x6C, 0x6C, 

    /* 0xCF */
    0x18, 0x18, 0xFF, 0x00, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xD0 */
    0x6C, 0x6C, 0x6C, 0x6C, 0xFF, 0x00, 0x00, 0x00, 

    /* 0xD1 */
    0x00, 0x00, 0xFF, 0x00, 0xFF, 0x18, 0x18, 0x18, 

    /* 0xD2 */
    0x00, 0x00, 0x00, 0x00, 0xFF, 0x6C, 0x6C, 0x6C, 

    /* 0xD3 */
    0x6C, 0x6C, 0x6C, 0x6C, 0xFC, 0x00, 0x00, 0x00, 

    /* 0xD4 */
    0x18, 0x18, 0xF8, 0x18, 0xF8, 0x00, 0x00, 0x00, 

    /* 0xD5 */
    0x00, 0x00, 0xF8, 0x18, 0xF8, 0x18, 0x18, 0x18, 

    /* 0xD6 */
    0x00, 0x00, 0x00, 0x00, 0xFC, 0x6C, 0x6C, 0x6C, 

    /* 0xD7 */
    0x6C, 0x6C, 0x6C, 0x6C, 0xFF, 0x6C, 0x6C, 0x6C, 

    /* 0xD8 */
    0x18, 0x18, 0xFF, 0x18, 0xFF, 0x18, 0x18, 0x18, 

    /* 0xD9 */
    0x18, 0x18, 0x18, 0x18, 0x1F, 0x00, 0x00, 0x00, 

    /* 0xDA */
    0x00, 0x00, 0x00, 0x00, 0xF8, 0x18, 0x18, 0x18, 

    /* 0xDB */
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 

    /* 0xDC */
    0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 

    /* 0xDD */
    0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 0x0F, 

    /* 0xDE */
    0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 0xF0, 

    /* 0xDF */
    0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 

    /* 0xE0 */
    0x00, 0x00, 0x6E, 0x3B, 0x13, 0x3B, 0x6E, 0x00, 

    /* 0xE1 */
    0x1E, 0x33, 0x33, 0x1B, 0x33, 0x63, 0x33, 0x00, 

    /* 0xE2 */
    0x7F, 0x63, 0x03, 0x03, 0x03, 0x03, 0x03, 0x00, 

    /* 0xE3 */
    0x00, 0x00, 0x7F, 0x36, 0x36, 0x36, 0x36, 0x00, 

    /* 0xE4 */
    0x7F, 0x63, 0x06, 0x0C, 0x06, 0x63, 0x7F, 0x00, 

    /* 0xE5 */
    0x00, 0x00, 0x7E, 0x1B, 0x1B, 0x1B, 0x0E, 0x00, 

    /* 0xE6 */
    0x00, 0x00, 0x66, 0x66, 0x66, 0x66, 0x3E, 0x03, 

    /* 0xE7 */
    0x00, 0x6E, 0x3B, 0x18, 0x18, 0x18, 0x18, 0x00, 

    /* 0xE8 */
    0x7E, 0x18, 0x3C, 0x66, 0x66, 0x3C, 0x18, 0x7E, 

    /* 0xE9 */
    0x1C, 0x36, 0x63, 0x7F, 0x63, 0x36, 0x1C, 0x00, 

    /* 0xEA */
    0x1C, 0x36, 0x63, 0x63, 0x36, 0x36, 0x77, 0x00, 

    /* 0xEB */
    0x70, 0x18, 0x30, 0x7C, 0x66, 0x66, 0x3C, 0x00, 

    /* 0xEC */
    0x00, 0x00, 0x7E, 0xDB, 0xDB, 0x7E, 0x00, 0x00, 

    /* 0xED */
    0x60, 0x30, 0x7E, 0xDB, 0xDB, 0x7E, 0x06, 0x03, 

    /* 0xEE */
    0x78, 0x0C, 0x06, 0x7E, 0x06, 0x0C, 0x78, 0x00, 

    /* 0xEF */
    0x00, 0x3E, 0x63, 0x63, 0x63, 0x63, 0x63, 0x00, 

    /* 0xF0 */
    0x00, 0x7F, 0x00, 0x7F, 0x00, 0x7F, 0x00, 0x00, 

    /* 0xF1 */
    0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x7E, 0x00, 

    /* 0xF2 */
    0x0C, 0x18, 0x30, 0x18, 0x0C, 0x00, 0x7E, 0x00, 

    /* 0xF3 */
    0x30, 0x18, 0x0C, 0x18, 0x30, 0x00, 0x7E, 0x00, 

    /* 0xF4 */
    0x70, 0xD8, 0xD8, 0x18, 0x18, 0x18, 0x18, 0x18, 

    /* 0xF5 */
    0x18, 0x18, 0x18, 0x18, 0x18, 0x1B, 0x1B, 0x0E, 

    /* 0xF6 */
    0x00, 0x18, 0x00, 0x7E, 0x00, 0x18, 0x00, 0x00, 

    /* 0xF7 */
    0x00, 0x6E, 0x3B, 0x00, 0x6E, 0x3B, 0x00, 0x00, 

    /* 0xF8 */
    0x1C, 0x36, 0x36, 0x1C, 0x00, 0x00, 0x00, 0x00, 

    /* 0xF9 */
    0x00, 0x00, 0x00, 0x18, 0x18, 0x00, 0x00, 0x00, 

    /* 0xFA */
    0x00, 0x00, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 

    /* 0xFB */
    0xF0, 0x30, 0x30, 0x30, 0x37, 0x36, 0x3C, 0x38, 

    /* 0xFC */
    0x36, 0x6C, 0x6C, 0x6C, 0x6C, 0x00, 0x00, 0x00, 

    /* 0xFD */
    0x1E, 0x30, 0x18, 0x0C, 0x3E, 0x00, 0x00, 0x00, 

    /* 0xFE */
    0x00, 0x00, 0x3C, 0x3C, 0x3C, 0x3C, 0x00, 0x00, 

    /* 0xFF */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
};

// Some useful flipping functions

void rotate_90_clockwise(const unsigned char *input, unsigned char *output, int size) {
    for (int row = 0; row < 8; row++) {
        unsigned char new_row = 0;
        for (int col = 0; col < 8; col++) {
            if (input[col] & (1 << row)) {
                new_row |= (1 << (7 - col));
            }
        }
        output[row] = new_row;
    }
}

void rotate_90_counterclockwise(const unsigned char *input, unsigned char *output, int size) {
    for (int row = 0; row < 8; row++) {
        unsigned char new_row = 0;
        for (int col = 0; col < 8; col++) {
            if (input[col] & (1 << (7 - row))) {
                new_row |= (1 << col);
            }
        }
        output[row] = new_row;
    }
}

void rotate_90_counterclockwise_flipped(const unsigned char *input, unsigned char *output, int size) {
    for (int row = 0; row < 8; row++) {
        unsigned char new_row = 0;
        for (int col = 7; col >= 0; col--) { // Fixed loop condition
            if (input[col] & (1 << (7 - row))) {
                new_row |= (1 << col);
            }
        }
        output[row] = new_row;
    }
}

void rotate_90_counterclockwise_flipped_2(const unsigned char *input, unsigned char *output, int size) {
    for (int row = 0; row < 8; row++) {
        unsigned char new_row = 0;
        for (int col = 7; col >= 0; col--) { // Iterate columns right to left
            if (input[col] & (1 << (7 - row))) {
                new_row |= (1 << (7 - col)); // Flip the column positions
            }
        }
        output[row] = new_row;
    }
}


int main() {
    int flip_bytes_in_a_row = 1; // set 1 with rotate_90_counterclockwise_flipped
    int num_chars = sizeof(font8x8) / 8; // Number of 8x8 characters
    unsigned char rotated[8];

    printf("const unsigned char font_vga8x8[] = {\n");

    for (int i = 0; i < num_chars; i++) {
        const unsigned char *char_bitmap = &font8x8[i * 8];

        // Rotate the character
        rotate_90_counterclockwise_flipped(char_bitmap, rotated, 8);

        // Print the rotated character
        printf("    ", i);
        if (flip_bytes_in_a_row) {
            for (int row = 7; row >= 0; row--) { // Invert row order
                printf(" 0x%02X%s", rotated[row], (row == 0) ? "," : ",");
            }
        } else {
            for (int row = 0; row < 8; row++) {
                printf(" 0x%02X%s", rotated[row], (row == 7) ? "," : ",");
            }
        }
        printf("  /* 0x%02X */\n", i);
    }

    printf("};\n");
    return 0;
}
